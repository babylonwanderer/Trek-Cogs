...
class Bartender:
    """Buy a drink at the bar with Discord Fleet Credits"""
    ...
    async def buy(self, ctx, amount : int, drink):
        """Buy a drink with Fleet Credits"""
        ...
        if econ.bank.can_spend(author, price):
            ...
            try:
                ...
                text_num = str(amount)
            await self.bot.reply("Sorry mate, you don't have enough Credits for {1} {2}.\n It costs {3}".format(text_num, drink, price))
    ...
    async def register(self, ctx):
        """Opens the bar and registers the bot into the Forbidden Angels bank."""
        ...
        if not bank.account_exists(botuser):
            ...
            await self.bot.say("{} already has an account at the Forbidden Angels bank.".format(botuser.mention))
